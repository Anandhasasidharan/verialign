...
def test_tool_grounder_numeric_mismatch():
    ...
    tool_calls = [{"name": "process_refund", "arguments": {"amount": 45}, "result": {"status": "processed", "amount": 45}}]
    status, conf, reason = grounder.ground("the refund was processed for $50", tool_calls)
...
def test_tool_grounder_no_mismatch():
    grounder = ToolGrounder()
    tool_calls = [{"name": "process_refund", "arguments": {"amount": 45}, "result": {"status": "processed", "amount": 45}}]
    status, conf, reason = grounder.ground("the refund was processed for $45", tool_calls)
...
def test_tool_grounder_non_tool_claim():
    grounder = ToolGrounder()
    tool_calls = [{"name": "process_refund", "arguments": {"amount": 45}, "result": {"status": "processed", "amount": 45}}]
    status, conf, reason = grounder.ground("Paris is the capital of France.", tool_calls)
    assert status is None
...
async def test_engine_tool_grounding_integration():
    ...
    result = await engine.verify("the refund was processed for $50", context=[], tool_calls=tool_calls)
...
async def test_response_handler_extracts_tool_calls_from_metadata():
    ...
    upstream = {"id": "t", "choices": [{"message": {"role": "assistant", "content": "refund was processed for $50"}}]}
    request = {
        "metadata": {
            "context": [],
            "tool_calls": [{"name": "process_refund", "arguments": {"amount": 45}, "result": {"status": "processed", "amount": 45}}]
...
async def test_response_handler_extracts_tool_calls_from_messages():
    ...
    request = {
        ...
        "messages": [
            ...
            {"role": "tool", "name": "process_refund", "content": '{"amount": 45, "status": "processed"}'}
        ]
    }
    augmented = await handler.augment(upstream, request)
    ...
    assert augmented.verification.claims[0].status in ("unsupported", "unclear", "supported")
