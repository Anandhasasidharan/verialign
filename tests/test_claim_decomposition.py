...
@pytest.mark.asyncio
async def test_decompose_compound_and() -> None:
    extractor = ClaimExtractor()
    claims = await extractor.extract(
        "VeriAlign is a proxy and it stores traces in SQLite.",
...
@pytest.mark.asyncio
async def test_no_decompose_without_and() -> None:
    extractor = ClaimExtractor()
    claims = await extractor.extract(
        "VeriAlign is a proxy. It stores traces in SQLite.",
        use_llm_fallback=False,
    )
    assert len(claims) == 2
...
@pytest.mark.asyncio
async def test_decompose_requires_factual_cues() -> None:
    extractor = ClaimExtractor()
...
@pytest.mark.asyncio
async def test_decompose_compound_with_three_parts() -> None:
    extractor = ClaimExtractor()
