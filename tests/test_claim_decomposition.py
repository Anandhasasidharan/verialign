...
async def test_decompose_compound_and():
    ...
    claims = await extractor.extract("VeriAlign is a proxy and it stores traces in SQLite.", use_llm_fallback=False)
...
async def test_no_decompose_without_and():
    extractor = ClaimExtractor()
    claims = await extractor.extract("VeriAlign is a proxy. It stores traces in SQLite.", use_llm_fallback=False)
    assert len(claims) == 2
...
async def test_decompose_requires_factual_cues():
    ...
    claims = await extractor.extract("The system is fast and reliable.", use_llm_fallback=False)
