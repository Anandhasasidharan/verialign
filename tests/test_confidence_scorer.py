...
class TestConfidenceScorer:
    ...
    def test_score_claim_uses_heuristic_when_no_logprobs(self) -> None:
        claim = "The Earth orbits the Sun."
        score = self.scorer.score_claim(claim, 0.8, logprobs=None)
        assert score.method == "heuristic"

    def test_score_claim_uses_logprobs_when_available(self) -> None:
        claim = "The Earth orbits the Sun."
        logprobs = [-0.1, -0.2, -0.15]
        score = self.scorer.score_claim(claim, 0.8, logprobs=logprobs)
        assert score.method == "logprob+overlap"

    def test_score_response_extracts_logprobs(self) -> None:
        response = {
            "choices": [
                {
                    "logprobs": {
    ...
    def test_score_response_handles_missing_logprobs(self) -> None:
        response = {"choices": [{}]}
        result = self.scorer.score_response(response)
        assert result is None

    def test_score_response_handles_empty_choices(self) -> None:
        response = {"choices": []}
        result = self.scorer.score_response(response)
        assert result is None
