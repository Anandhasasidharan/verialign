...
_TOOL_HINT = re.compile(r"\b(refund|payment|transfer|order|amount|processed|tool)\b", re.IGNORECASE)
    ...
    def normalize_tool_calls(self, raw) -> list[dict]:
        ...
        for item in raw:
            if isinstance(item, dict):
                ...
                normalized.append({"name": str(name), "arguments": args, "result": result})
        return normalized

    def ground(self, claim: str, tool_calls: list[dict]) -> tuple[str | None, float, str | None]:
        ...
        claim_has_hint = bool(_TOOL_HINT.search(claim) or "$" in claim or _NUMBER.search(claim))
        ...
        for tc in tool_calls:
            result_str = json.dumps(tc.get("result")) if not isinstance(tc.get("result"), str) else str(tc.get("result"))
            args_str = json.dumps(tc.get("arguments")) if not isinstance(tc.get("arguments"), str) else str(tc.get("arguments"))
                ...
                for cn in claim_numbers:
                    if cn not in tool_numbers:
                        ...
                        if tc.get("name") and tc["name"].lower() in claim.lower():
                            return "unsupported", 0.85, f"claim amount {cn} != tool {tc['name']} result {tool_numbers}"
                        # Generic amount mismatch
                        if len(tool_numbers) == 1 and len(claim_numbers) == 1:
                            return "unsupported", 0.85, f"amount mismatch claim {cn} vs tool {tool_numbers[0]}"
                        # If claim number clearly numeric and tool has different numeric, unclear
                        return "unsupported", 0.75, f"numeric mismatch {cn} vs {tool_numbers}"
            ...
            if "processed" in claim_lower and ("failed" in result_lower or "error" in result_lower):
                return "unsupported", 0.8, "status mismatch: claim says processed but tool result indicates failure"
